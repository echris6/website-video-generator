...
const app = express();
const PORT = 3005; // Different port for smooth Step 2
...
console.log(`
╔════════════════════════════════════════════════════╗
║        CHATBOT STEP 2 - SMOOTH INTERACTION        ║
║              Running on port ${PORT}                    ║
║                                                    ║
║  🖱️ Step 2: Cursor → Click → Widget Opens         ║
...
function customizeWebsite(niche, businessName) {
    ...
    if (cursorBase64) {
        ...
        // STEP 2: Human-like cursor movement + clicking interaction (IMMEDIATE START)
        window.updateCursorForFrame = function(frameIndex, totalFrames) {
            ...
            if (chatbotInfo.found) {
                // Phase 1 (0-3 seconds): IMMEDIATE movement to chatbot
                if (progress < 0.1) { // 0-3 seconds: Move immediately to chatbot
                    const moveProgress = progress / 0.1; // 0 to 1 over 3 seconds
                    const startX = 800;
                    const startY = 400;
                ...
                // Phase 2 (3-5 seconds): Click chatbot button
                else if (progress < 0.167) { // 5/30 = 0.167
                    window.moveCursorTo(chatbotInfo.x, chatbotInfo.y);
                    
                    // Trigger click once at 4 seconds
                    if (progress >= 0.133 && progress < 0.15 && !window.hasClicked) {
                        window.clickChatbotButton();
                        window.hasClicked = true;
                    }
                }
                // Phase 3 (5-30 seconds): Stay at chatbot with widget open
                else {
                    window.moveCursorTo(chatbotInfo.x, chatbotInfo.y);
                }
            } else {
...
async function generateVideo(businessName, niche = 'real_estate') {
    ...
    const videoFilename = `chatbot_step2_smooth_${businessName.toLowerCase().replace(/\s+/g, '_')}_${timestamp}.mp4`;
    const framesDir = path.join(__dirname, 'frames', `temp_${timestamp}`);
    
    console.log(`🎥 CHATBOT STEP 2 - SMOOTH INTERACTION`);
    console.log(`📋 Business: ${businessName}`);
    console.log(`🏢 Niche: ${niche}`);
    console.log(`🖱️ Cursor: IMMEDIATE Move → Click (4s) → Widget Opens`);
    ...
    try {
        ...
        *::before, *::after {
        ...
        console.log(`📐 Step 2: Cursor movement + clicking interaction`);
...
app.post('/generate-video', async (req, res) => {
    try {
        const { businessName = 'Step 2 Smooth Test', niche = 'real_estate' } = req.body;
...
// Start server
app.listen(PORT, () => {
    console.log(`👉 Ready! Test with: curl -X POST http://localhost:${PORT}/generate-video -H "Content-Type: application/json" -d '{"businessName":"Step 2 Smooth Test","niche":"real_estate"}'`);
}); 