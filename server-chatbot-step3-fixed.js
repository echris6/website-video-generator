...
console.log('⚠️  server-chatbot-step3-fixed.js is DISABLED to prevent multiple video generation');
...
console.log(`
╔════════════════════════════════════════════════════╗
║        CHATBOT STEP 3 - FAST TYPING FIXED         ║
║              Running on port ${PORT}                    ║
║                                                    ║
║  🖱️ Step 3: Fast Cursor → Click → Type Message    ║
...
async function generateVideo(businessName, niche = 'real_estate') {
    ...
    const videoFilename = `chatbot_step3_fast_${businessName.toLowerCase().replace(/\s+/g, '_')}_${timestamp}.mp4`;
    const framesDir = path.join(__dirname, 'frames', `temp_${timestamp}`);
    
    console.log(`🎥 CHATBOT STEP 3 - FAST TYPING INTERACTION`);
    console.log(`📋 Business: ${businessName}`);
    console.log(`🏢 Niche: ${niche}`);
    console.log(`🖱️ Cursor: FAST Move (2s) → Click (2.5s) → FAST to Input (1s) → Type "I want to buy a house" (6s)`);
    ...
    try {
        ...
        *::before, *::after {
        ...
        /* Ensure chat input is wide enough to display "I want to buy a house" */
        .chat-input input {
        ...
        /* Ensure the input container is wide enough */
        .chat-input {
            width: 100% !important;
        ...
        await page.evaluate(() => {
            ...
            window.updateCursorForFrame = function(frameIndex, totalFrames) {
                ...
                if (chatbotInfo.found) {
                    // Phase 1 (0-2 seconds): FASTER movement to chatbot
                    if (progress < 0.2) { // 0-2 seconds: Move faster to chatbot (2/10 = 0.2)
                        const moveProgress = progress / 0.2; // 0 to 1 over 2 seconds
                        const startX = 800;
                        const startY = 400;
                        const endX = chatbotInfo.x;
                        const endY = chatbotInfo.y;
                    ...
                    // Phase 2 (2-3 seconds): Quick click chatbot button
                    else if (progress < 0.3) { // 3/10 = 0.3
                        window.moveCursorTo(chatbotInfo.x, chatbotInfo.y);
                        
                        // Trigger click once at 2.5 seconds
                        if (progress >= 0.25 && progress < 0.27 && !window.hasClicked) {
                            window.clickChatbotButton();
                            window.hasClicked = true;
                        }
                    }
                    // Phase 3 (3-4 seconds): MUCH FASTER move to chat input field
                    else if (progress < 0.4 && chatInputInfo.found) { // 4/10 = 0.4
                        const moveProgress = (progress - 0.3) / 0.1; // 0 to 1 over 1 second
                        const startX = chatbotInfo.x;
                        const startY = chatbotInfo.y;
                        const endX = chatInputInfo.x;
                        const endY = chatInputInfo.y;
                    ...
                    // Phase 4 (4-10 seconds): MUCH FASTER typing with line break
                    else if (progress < 1.0 && chatInputInfo.found) { // 10/10 = 1.0
                        window.moveCursorTo(chatInputInfo.x, chatInputInfo.y);
                        
                        // Type message - Real estate inquiry as requested
                        const message = "I want to buy a house";
                        const typingProgress = (progress - 0.4) / 0.6; // 0 to 1 over 6 seconds (4-10s)
                        const targetLength = Math.floor(typingProgress * message.length);
        ...
        // 🔍 VERIFICATION: Check if functions are now loaded
        ...
        console.log(`✅ Functions injected directly:`, functionsExist);
        ...
        console.log(`📐 Step 3: Fast cursor movement + click + type short message (10 seconds)`);
        ...
        return {
            ...
            chatbotInfo,
            step: 3,
            description: "Fast 10-second chatbot interaction: cursor + click + type 'I want to buy a house'"
        };
...
app.post('/generate-video', async (req, res) => {
    try {
        const { businessName = 'Step 3 Fast Typing Test', niche = 'real_estate' } = req.body;
...
// Start server
app.listen(PORT, () => {
    console.log(`👉 Ready! Test with: curl -X POST http://localhost:${PORT}/generate-video -H "Content-Type: application/json" -d '{"businessName":"Step 3 Fast Typing Test","niche":"real_estate"}'`);
}); 