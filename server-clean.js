...
console.log(`
╔════════════════════════════════════════════════════╗
║              CLEAN VIDEO GENERATOR                ║
║              Running on port ${PORT}                    ║
║                                                    ║
║  🎯 Simple & Reliable Screenshot Cropping          ║
...
async function generateVideo(businessName, niche = 'real_estate') {
    ...
    const videoFilename = `${businessName.toLowerCase().replace(/\s+/g, '_')}_${timestamp}.mp4`;
    const framesDir = path.join(__dirname, 'frames', `temp_${timestamp}`);
    
    console.log(`🎥 CLEAN VIDEO GENERATION`);
...
app.post('/generate-video', async (req, res) => {
    try {
        const { businessName = 'Clean Test', niche = 'real_estate' } = req.body;
...
// Start server
app.listen(PORT, () => {
    console.log(`👉 Ready! Test with: curl -X POST http://localhost:${PORT}/generate-video -H "Content-Type: application/json" -d '{"businessName":"Test Demo","niche":"real_estate"}'`);
});
